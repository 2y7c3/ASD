...
class PromptProcessorOutput:
    ...
    def get_text_embeddings_perp_neg(
        ...
        #print(side_emb.shape, front_emb.shape, back_emb.shape, overhead_emb.shape)
        ...
        for idx, ele, azi, dis in zip(
            ...
            if idx.item() == 3:  # overhead view
                ...
                if torch.abs(azi) < 90:
                    ...
                    neg_guidance_weights += [
                        #1.0,
                        0.0 if r_inter > 0.8 else math.exp(-r_inter * 2) * (-2),
                        0.0 if r_inter < 0.2 else math.exp(-(1-r_inter) * 10) * (-2)
                    ...
                    neg_guidance_weights += [
                        0.0 if r_inter > 0.8 else math.exp(-r_inter * 10) * (-2) / 2,
                        -2.0,
...
class PromptProcessor(BaseObject):
    ...
    def load_from_cache(self, prompt):
        cache_path = os.path.join(
            ...
            (
                f"{hash_prompt(self.cfg.pretrained_model_name_or_path, prompt)}.pt"
                if self.cfg.LoRA_path is None else
                f"{hash_prompt(self.cfg.pretrained_model_name_or_path, prompt)}_lora.pt"
            ),
        )

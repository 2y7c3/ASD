...
class RandomCameraDataModuleConfig:
    ...
    camera_perturb: float = 0. #0.05
    center_perturb: float = 0. #0.05
    up_perturb: float = 0.0 #0.01
    ...
    batch_uniform_azimuth: bool = False #True
    progressive_until: int = 0  # progressive ranges for elevation, azimuth, r, fovy

    max_camera_distance_range: Tuple[float, float] = (5.2, 5.5)
    max_azimuth_range: Tuple[float, float] = (-180, 180)
    max_elevation_range: Tuple[float, float] = (-30, 45)
    max_fovy_range: Tuple[float, float] = (0.16, 0.60)
    fovy_scale_up_factor: Tuple[float, float] = (0.75, 1.1)
    phi_scale_up_factor: float = 1.5
    scale_up_factor: float = 0.95
    rand_cam_gamma: float = 1.0

    rays_d_normalize:bool = False
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def progressive_view(self, global_step):
        ...
        self.cfg.fovy_range[0] = max(self.cfg.max_fovy_range[0], self.cfg.fovy_range[0] * self.cfg.fovy_scale_up_factor[0])
        self.cfg.fovy_range[1] = min(self.cfg.max_fovy_range[1], self.cfg.fovy_range[1] * self.cfg.fovy_scale_up_factor[1])

        self.cfg.camera_distance_range[1] = max(self.cfg.max_camera_distance_range[1], self.cfg.camera_distance_range[1] * self.cfg.scale_up_factor)
        self.cfg.camera_distance_range[0] = max(self.cfg.max_camera_distance_range[0], self.cfg.camera_distance_range[0] * self.cfg.scale_up_factor)

        self.cfg.elevation_range[1] = min(self.cfg.max_elevation_range[1], self.cfg.elevation_range[1] * self.cfg.phi_scale_up_factor)
        self.cfg.elevation_range[0] = max(self.cfg.max_elevation_range[0], self.cfg.elevation_range[0] * self.cfg.phi_scale_up_factor)
        ...
        print('scale up fovy_range to:', self.cfg.fovy_range)
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if random.random() < 0.5:
            ...
            #)
            elevation_deg = gen_random_pos(self.batch_size, self.elevation_range, gamma=self.cfg.rand_cam_gamma)
            ...
            elevation = torch.asin(
                2
                * gen_random_pos(self.batch_size, elevation_range_percent, gamma=self.cfg.rand_cam_gamma)
                #(
                #    torch.rand(self.batch_size)
        ...
        if self.cfg.batch_uniform_azimuth:
            ...
            #)
            azimuth_deg = gen_random_pos(self.batch_size, self.azimuth_range, gamma=self.cfg.rand_cam_gamma)
        ...
        #)
        camera_distances = gen_random_pos(self.batch_size, camera_distance_range, gamma=self.cfg.rand_cam_gamma)
        ...
        fovy = torch.rand(self.batch_size) * (self.cfg.fovy_range[1] - self.cfg.fovy_range[0]) + self.cfg.fovy_range[0]
        fovy = focal2fov(fov2focal(fovy, self.cfg.height), self.cfg.width)
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        fovy = torch.full_like(elevation_deg, self.cfg.eval_fovy_deg)
        #torch.tensor(self.cfg.eval_fovy_deg, dtype=torch.float32).reshape(-1) #random.random() * (self.cfg.fovy_range[1] - self.cfg.fovy_range[0]) + self.cfg.fovy_range[0]
        fovy = focal2fov(fov2focal(fovy, self.cfg.eval_height), self.cfg.eval_width)

...
def load_lora_weights(pipeline, checkpoint_path, device, dtype, multiplier=1.00, patch_unet=True, patch_text=True):
    ...
    for layer, elems in updates.items():
        ...
        if "text" in layer:
            if patch_text == False: continue
            layer_infos = layer.split(LORA_PREFIX_TEXT_ENCODER + "_")[-1].split("_")
            curr_layer = pipeline.text_encoder
        else:
            if patch_unet == False: continue

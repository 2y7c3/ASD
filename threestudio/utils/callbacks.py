...
class ModelTestCallback(Callback):
    def __init__(self, start_iter, interval, datamodule):
        super().__init__()
        self.start_iter = start_iter
        self.interval = interval  # interval could be representing epochs or steps
        self.dm = datamodule
    
    #@rank_zero_only
    def on_train_batch_end(self, trainer, pl_module, *args, **kwargs):
        if (pl_module.global_step + 1) >= self.start_iter and (pl_module.global_step + 1) % self.interval == 0:
            trainer.test(pl_module, datamodule=self.dm)
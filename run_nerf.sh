...
tmux new-session -d -s nohup_nerf
...
while IFS= read -r line; do
  ...
  tmux new-window -t nohup_nerf -n "$gpu_id+"
  tmux send-keys -t nohup_nerf:"$gpu_id+" "source ../hf_profile" C-m
  tmux send-keys -t nohup_nerf:"$gpu_id+" "python launch.py --gpu $gpu_id --config configs/test_nerf.yaml --train system.prompt_processor.prompt='$others' system.prompt_processor.use_perp_neg=$use_perpneg" C-m
  #nohup python launch.py --gpu $gpu_id --config configs/test.yaml --train system.prompt_processor.prompt="$others" system.prompt_processor.use_perp_neg=$use_perpneg system.geometry.init_prompt="$others"> "logs/gs_$others.log" 2>&1 &
  #pid=($!)
  #echo "Job id $pid"
  sleep 300
done < "$PARAMS_FILE"